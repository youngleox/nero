...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.embedding = sn(nn.Linear(1000, self.embedding_size,bias=True))
        ...
        self.bn_img = nn.BatchNorm2d(nf,affine=True) # Yang disabled bn affine
...
class ResBlock(nn.Module):
    ...
    def forward(self, input_x, cond):
        ...
        out = self.relu_0(self.bn_0(input_x, cond))
        out = F.interpolate(out, scale_factor=2, mode='nearest')
        out = self.conv_0(out)
        out = self.conv_1(self.relu_1(self.bn_1(out, cond)))
        out += skip
        return out
