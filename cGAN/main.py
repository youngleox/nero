...
parser.add_argument('--bare', action='store_true', help='strip out bn and sn')

parser.add_argument('--n', type=int, default=8, help='number of workers')
...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.bsz, shuffle=True, num_workers=args.n)

trainsubset = torch.utils.data.Subset(trainset, range(0,10000)) # for purposes of FID calculation
trainsubloader = torch.utils.data.DataLoader(trainsubset, batch_size=args.bsz, shuffle=True, num_workers=args.n)

testset = torchvision.datasets.CIFAR10(root='./data', train=False, download=True, transform=transform)
testloader = torch.utils.data.DataLoader(testset, batch_size=args.bsz, shuffle=False, num_workers=args.n)
...
if args.bare:
    print("BN and SN are switched OFF.")
    from architecture_bare.generator import Generator
    from architecture_bare.discriminator import Discriminator
else:
    print("BN and SN are switched ON.")
...
if args.optim == 'adam':
    optG = optim.Adam(netG.parameters(), lr=args.initial_lr,betas=(0.0,0.999))
    optD = optim.Adam(netD.parameters(), lr=args.initial_lr,betas=(0.9,0.999))
...
def train():
    ...
    for it, (data, label) in enumerate(tqdm(trainloader)):
        ...
        # warmup(optD,1,epoch,args.initial_lr,it,len(trainloader))
        # warmup(optG,1,epoch,args.initial_lr,it,len(trainloader))
...
def warmup(optimizer,warmup_epoch,epoch,lr,n,N):
    new_lr = float(n + N * (epoch)) / float(warmup_epoch * N) * lr
...
for epoch in range(0, args.epochs):
    ...
    if epoch==100:
        print("decaying learning rate by 10")
