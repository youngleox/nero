...
parser.add_argument('--lr-decay-epoch', default='30,60', type=str,
                    help='lr decay epochs for step schedule')
parser.add_argument('--lr-decay', default=0.1, type=float,
                    help='lr decay coefficient for step schedule')
...
def train(train_loader, model, criterion, optimizer, epoch, args, writer,scheduler=None):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        if epoch < args.warmup_epoch:
            warmup(optimizer,args.warmup_epoch,epoch,args.lr,i,len(train_loader))
    ...
    return top1.avg, top5.avg, losses.avg, batch_time.avg, scheduler
...
def adjust_learning_rate(optimizer, epoch, lr, lr_decay_epoch,lr_decay):
    """Sets the learning rate to the initial LR decayed by 10 every 30 epochs"""
    for i in lr_decay_epoch:
        if epoch == i:
            lr = lr * lr_decay
