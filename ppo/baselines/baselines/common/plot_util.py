...
COLORS = ['royalblue', 'olivedrab', 'purple','darkorange','blue', 'green', 'purple', 'red', 'cyan', 'magenta', 'yellow', 'black', 'pink',
    ...
    for (isplit, sk) in enumerate((sk2r.keys())):
        ...
        if average_group:
            ...
            for group in (groups):
                ...
                l, = axarr[idx_row][idx_col].plot(usex, ymean, color=color,zorder=z)
                print('mean: {}, std: {}'.format(ymean[-1],ystd[-1]))
                g2l[group] = l
                if shaded_err:
                    #ax.fill_between(usex, ymean - ystderr, ymean + ystderr, color=color, alpha=.4)
                    ax.fill_between(usex, ymin, ymax, color=color, alpha=.2,zorder=z-1, edgecolor="b", linewidth=0.0)
                if shaded_std:
        ...
        plt.tight_layout()
        if any(g2l.keys()) and 1:
            ax.legend(
                g2l.values(),
                # ['%s (%i runs)'%(g, g2c[g]) for g in g2l] if average_group else g2l.keys(),
                ['%s'%(g) for g in g2l] if average_group else g2l.keys(),
                loc=2 if legend_outside else 3,
                bbox_to_anchor=(1,1) if legend_outside else (0.62,0.1),
                framealpha=0.2,
                edgecolor='black').set_zorder(10002)
        ax.set_title(sk)
