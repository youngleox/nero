...
class Nero_abl(Optimizer):

    def __init__(self, params, lr=0.01, beta=0.999, c1=True, c2=True):
        self.beta = beta
        self.c1 = c1
        self.c2 = c2
        defaults = dict(lr=lr)
        super(Nero_abl, self).__init__(params, defaults)
        print('ei_balance: {}, fixed norm: {}'.format(c1,c2))
        for group in self.param_groups:
            for p in group['params']:
                if p.dim() > 1:
                    if self.c1:
